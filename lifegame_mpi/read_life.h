...
void mpi_read_initial_file(int argc, char *argv[], char *filename, double prob, int *seed, int *it_max, int *m, int *n,
                           int **grid, double *start_time, int *nproc, int *iproc, int *size_to_work, int *iproc_prev,
                           int *iproc_next);

void recieve_all_data(int nproc, int iproc, int *it_max, int *m, int *n, int **grid);

void initialization(int argc, char *const *argv, char *filename, double prob, int *seed, int nproc, int *it_max, int *m,
                    int *n, int **grid, double *start_time);
...
void
recieve_all_data(int nproc, int iproc, int *it_max, int *m, int *n, int **grid) {// Data to be received: it_max, n, m,
    ...
    (*grid) = (int *) malloc((size_to_work + 2) * (*m + 2) * sizeof(int));
    guardian("[1]: Allocated grid");
    char msg[200];
    sprintf(msg, "[1]: Receiving grid of: %d x %d", size_to_work, (*m));
    guardian(msg);
    MPI_Recv((*grid) + (*m + 2), size_to_work * (*m + 2),
             MPI_INT, 0, tag_initial_grid, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
